...
LABEL_FOLDER = "pose_TFLite_model"
LABELMAP_NAME = 'labelmap.txt'
...
# PATH_TO_CKPT = os.path.join(CWD_PATH,LABEL_FOLDER,GRAPH_NAME)

# Path to label map file
PATH_TO_LABELS = os.path.join(CWD_PATH,LABEL_FOLDER,LABELMAP_NAME)
...
#         self.container = av.open(self.drone.get_video_stream())
    ...
    def process_frame(self, frame):
        ...
        ##################
        #########FUNCTIONS
        def draw_arrows(frame,imW=None, imH=None, xoff=None, yoff=None):
        ...
        def DetectPosesInImage(img,image_height=None,image_width=None,input_shape=None):
            ...
            assert (img.shape == tuple(input_shape[1:]))
        ...
        frame_resized = DetectPosesInImage(frame_resized,image_height, image_width, input_shape)
        ...
        for idx,label in enumerate(labels):
            if keypoint_dict[label][-1] >= min_thresh:
                ...
                x = round((keypoint_dict[label][1][1]/imW)*950)
                y = round((keypoint_dict[label][1][0]/imH)*720)
        ...
        image = draw_arrows(image,950, 720, xoff, yoff)
